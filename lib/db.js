/**
 * 谱子数据存储层（Neon Postgres + 内存降级）
 * - Vercel 部署时通过 Neon 集成自动注入连接串（DATABASE_URL 等）
 * - 本地开发未配置数据库时降级为内存存储（数据不持久，仅用于开发调试）
 * - 主存储格式：MusicXML 4.0 文本（musicxml TEXT 列）
 * - 兼容：notes JSONB 列保留（向后兼容旧数据），读取时优先 musicxml
 */

import { neon } from '@neondatabase/serverless';
import { sanitizeSheet } from './sheet';
import { notesToXml, xmlToNotes } from './musicxml';
...
async function ensureTable() {
  ...
  // 老表迁移：补 musicxml / audio_url 列
  for (const stmt of [
    "ALTER TABLE sheets ADD COLUMN IF NOT EXISTS musicxml TEXT NOT NULL DEFAULT ''",
    "ALTER TABLE sheets ADD COLUMN IF NOT EXISTS audio_url TEXT NOT NULL DEFAULT ''",
  ]) {
    try {
      await sql.unsafe(stmt);
    } catch { /* 列已存在或并发迁移冲突，忽略 */ }
  }
...
function seedData() {
  ...
  // 服务端直接生成 MusicXML（musicxml-io 无 Node 依赖）
  return raw.map((r) => ({
    ...r,
    musicxml: notesToXml(r.notes, { name: r.name, key: r.key, timeSignature: r.timeSignature, bpm: r.bpm }),
  }));
}

// ============================================
// 记录 → 前端返回格式（优先从 musicxml 解析 notes）
// ============================================

function toMeta(rec) {
  // 从 musicxml 解析 notes（如果 notes 空但 musicxml 有值）
  let notes = rec.notes;
  if ((!notes || !notes.length) && rec.musicxml) {
    try { ({ notes } = xmlToNotes(rec.musicxml)); } catch { /* 忽略解析失败 */ }
  }
  return {
    id: rec.id,
    name: rec.name,
...
function toFullSheet(rec) {
  // 优先 musicxml 解析 notes；兼容旧数据仅有 notes 的情况
  let notes = rec.notes;
  let musicxml = rec.musicxml || '';
  if (!musicxml && notes?.length) {
    musicxml = notesToXml(notes, { name: rec.name, key: rec.key, timeSignature: rec.timeSignature, bpm: rec.bpm });
  }
  if ((!notes || !notes.length) && musicxml) {
    try { ({ notes } = xmlToNotes(musicxml)); } catch { /* 忽略 */ }
  }
  return {
    id: rec.id, name: rec.name, key: rec.key,
    timeSignature: rec.timeSignature, bpm: rec.bpm,
    notes: notes || [],
    musicxml,
    audioUrl: rec.audioUrl || '',
  };
...
async function seedIfEmpty(sql) {
  ...
  for (const data of seedData()) {
    await sql`
      INSERT INTO sheets (id, name, sheet_key, time_signature, bpm, notes, musicxml, audio_url)
      VALUES (${data.id}, ${data.name}, ${data.key}, ${data.timeSignature}, ${data.bpm},
              ${JSON.stringify(data.notes)}::jsonb, ${data.musicxml}, '')
...
export async function listSheets() {
  ...
  const rows = await sql`
    SELECT id, name, sheet_key, time_signature, bpm, notes, musicxml, audio_url, updated_at
  ...
  return rows.map((r) => toMeta({
    ...
    notes: r.notes, musicxml: r.musicxml, audioUrl: r.audio_url, updatedAt: r.updated_at,
  }));
}

/** 获取单个谱子（含完整音符 + musicxml） */
export async function getSheet(id) {
  if (!(await checkDb())) {
    const rec = memory.get(id);
    if (!rec) return null;
    return toFullSheet(rec);
  }
  const sql = sqlInstance();
  await ensureTable();
  const rows = await sql`
    SELECT id, name, sheet_key, time_signature, bpm, notes, musicxml, audio_url
  ...
  return toFullSheet({
    id: r.id, name: r.name, key: r.sheet_key,
    timeSignature: r.time_signature, bpm: r.bpm,
    notes: r.notes, musicxml: r.musicxml, audioUrl: r.audio_url || '',
  });
}
...
export async function createSheet(body) {
  ...
  const id = genId();
  // 生成 MusicXML（从 notes + meta）
  const musicxml = notesToXml(clean.notes, { name: clean.name, key: clean.key, timeSignature: clean.timeSignature, bpm: clean.bpm });
  const rec = { id, ...clean, musicxml };
  if (!(await checkDb())) {
    memory.set(id, { ...rec, updatedAt: new Date().toISOString() });
    return getSheet(id);
  }
  const sql = sqlInstance();
  await ensureTable();
  await sql`
    INSERT INTO sheets (id, name, sheet_key, time_signature, bpm, notes, musicxml, audio_url)
    VALUES (${id}, ${clean.name}, ${clean.key}, ${clean.timeSignature}, ${clean.bpm},
            ${JSON.stringify(clean.notes)}::jsonb, ${musicxml}, ${clean.audioUrl || ''})
  `;
  return getSheet(id);
}
...
export async function updateSheet(id, body) {
  ...
  if (!(await checkDb())) {
    const rec = memory.get(id);
    if (!rec) return null;
    Object.assign(rec, { ...clean, musicxml, updatedAt: new Date().toISOString() });
    return getSheet(id);
  }
  const sql = sqlInstance();
  await ensureTable();
